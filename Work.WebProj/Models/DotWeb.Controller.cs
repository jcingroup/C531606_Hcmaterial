...
namespace DotWeb.Controller
{
    ...
    {
        //protected string IP;
        protected C53A0_HcmaterialEntities db0;
        ...
        protected static C53A0_HcmaterialEntities getDB0()
        {
    ...
    public abstract class AdminController : SourceController
    {
        ...
        /// <returns></returns>
        protected bool CompressImage(Stream sFile, string dFile, int flag = 90, int size = 300, bool sfsc = true)
        {
    ...
    public abstract class WebUserController : SourceController
    {
        ...
        public string ImgSrc(string AreaName, string ContorllerName, Int32 Id, String FilesKind, string ImageSizeTRype)
        {
            ...
            String SearchPath = string.IsNullOrWhiteSpace(ImageSizeTRype) ? String.Format(getImg_path_tpl, AreaName, ContorllerName, Id, FilesKind) : String.Format(sysUpFilePathTpl, AreaName, ContorllerName, Id, FilesKind, ImgSizeString);
            String FolderPth = Server.MapPath(SearchPath);
            ...
            {
                String[] SFiles = Directory.GetFiles(FolderPth);
                var directory = new DirectoryInfo(FolderPth);
                //判斷 有沒有檔案存在的時候, 忽略隱藏屬性的檔案
                var files = directory.GetFiles(string.Format("*.{0}", filters)).Where(f => !f.Attributes.HasFlag(FileAttributes.Hidden)).ToList();
                var obj = files.FirstOrDefault();

                if (obj != null)
                {
                    return Url.Content(SearchPath) + "/" + obj.Name;
                }
                else
                {
        ...
        public ValidateResponse ValidateCaptcha(string response)
        {
            ...
            var reply =
                client.DownloadString(
                    string.Format("https://www.google.com/recaptcha/api/siteverify?secret={0}&response={1}", "6LeDvxoTAAAAAOytfdH8tYSN6GuTFU_cRZHCwbv7", response));
